  ...
  return (
    <div className="bg-white border-b border-slate-200 px-3 py-2 shrink-0">
      <div className="flex items-center gap-2 mb-1.5">
        <span className="text-xs font-bold text-indigo-600 uppercase tracking-wider">⚡ Quick Items</span>
        ...
        {quickItems.map((item) => (
          ...
          >
            ...
            <div className="flex items-center justify-between mt-0.5">
              <span className="text-xs font-mono text-indigo-600 font-bold">{paise(item.price_paise)}</span>
              <span className="w-5 h-5 rounded-lg bg-indigo-100 text-indigo-600 flex items-center justify-center text-[10px] font-black">+</span>
            </div>
            {item.is_veg && (
