...
export function PosCartDrawer({
  ...
  return (
    <aside
      ...
      } md:flex md:static w-full md:w-80 lg:w-96 bg-white border-l border-slate-200 flex-col shrink-0 shadow-lg overflow-y-auto`}
    >
        ...
        <div>
          <h2 className="font-extrabold text-sm text-slate-900 flex items-center gap-2">
            ...
            {orderType === "dine_in" && selectedTable && (
              <span className="px-2.5 py-0.5 rounded-full text-xs bg-amber-100 text-amber-900 font-bold border border-amber-300">
                {selectedTable.label}
              </span>
            )}
          </h2>
          <p className="text-xs text-slate-500 mt-0.5">{cart.length} item lines</p>
        </div>
        <div className="flex gap-2 items-center">
          {cart.length > 0 && (
            <button
              type="button"
              onClick={clearCart}
              className="text-xs text-red-600 hover:text-red-700 font-bold cursor-pointer transition-colors p-1"
            >
              Clear
            </button>
          )}
          {mobileCartOpen && (
            <button
              ...
              className="md:hidden px-3 py-1.5 bg-slate-100 text-slate-800 rounded-xl text-xs font-bold min-h-[44px]"
            >
              ✕ Close
            </button>
          )}
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {(
            ...
            <button
              ...
              className={`py-2 rounded-xl text-xs font-black border cursor-pointer transition-all active:scale-95 ${
                orderType === o.id
      ...
      <div className="p-3.5 bg-white border-t border-slate-200 space-y-3 shrink-0">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {[0, 5, 10, 15, 20].map((pct) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-mono font-bold cursor-pointer transition-all active:scale-95 min-h-[36px] ${
            ...
            <div className="flex items-center gap-1 bg-slate-50 px-2 py-1 rounded-xl border border-slate-200 min-h-[36px]">
        ...
        <div className="space-y-1 text-xs pt-2 border-t border-slate-200">
          <div className="flex justify-between text-slate-500 font-medium">
            <span>Subtotal:</span>
            <span className="font-mono font-bold">{paise(subtotalPaise)}</span>
          </div>
          {discountPaise > 0 && (
          ...
          <div className="flex justify-between text-sm sm:text-base font-black text-slate-900 pt-1">
            ...
            <span className="text-indigo-600 font-mono font-extrabold">{paise(finalTotalPaise)}</span>
          </div>
        </div>

        {/* Split Bill per Guest Calculator */}
        <div className="p-2.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-1.5 text-xs">
        ...
        <div className="space-y-2 pt-1">
            ...
            {(
              ...
              return (
                <button
                  ...
                  className={`py-2 rounded-xl text-xs font-black border cursor-pointer transition-all active:scale-95 min-h-[44px] ${
                    isSelected
          ...
          {isSplitTender && (
            <div className="p-3 rounded-2xl bg-slate-50 border border-slate-200 space-y-2 animate-in fade-in">
              <div className="text-xs font-bold text-indigo-600 uppercase tracking-wider">Mixed Tender Split:</div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-slate-300 rounded-xl p-2 text-xs text-slate-900 font-mono focus:outline-none focus:border-indigo-500 font-bold"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-slate-300 rounded-xl p-2 text-xs text-emerald-600 font-mono focus:outline-none focus:border-emerald-500 font-bold"
                  />
                </div>
              </div>
            </div>
        ...
        {!isSplitTender && (
          <div className="pt-1">
            ...
            <input
              ...
              className="w-full bg-white border border-slate-300 rounded-xl p-2.5 text-xs text-slate-900 font-mono focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 min-h-[44px] transition-all font-bold shadow-sm"
            />
              ...
              {[
                ...
                .map((val) => (
                  <button
                    ...
                    className="px-3 py-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl text-xs font-mono font-bold text-slate-800 cursor-pointer shrink-0 min-h-[36px] active:scale-95 transition-all shadow-sm"
                  >
                    ₹{val}
                  </button>
                ))}
        ...
        <div className="flex gap-2 pt-2">
          <button
            ...
            className="flex-1 py-3.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs cursor-pointer border border-slate-300 disabled:opacity-40 min-h-[44px] transition-all active:scale-95 shadow-sm"
          >
            KOT & Bill Later
          </button>
          <button
            ...
            disabled={isSettling || cart.length === 0}
            className="flex-1 py-3.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-black text-xs cursor-pointer shadow-lg shadow-indigo-600/20 border border-indigo-500 disabled:opacity-40 min-h-[44px] transition-all active:scale-95 uppercase tracking-wider"
          >
