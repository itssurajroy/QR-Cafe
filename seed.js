...
const ITEMS = [
  ...
  // Desserts
  { cat: "Desserts", name: "Gulab Jamun (2 pcs)", desc: "Warm milk dumplings in sugar syrup", price: 9000, is_veg: true, img: "https://images.unsplash.com/photo-1555127394-01362e92c2da?auto=format&fit=crop&w=800&q=80" },
];

async function run() {
  // Find WAH JI WAH restaurant
  const { data: rest, error: restErr } = await supabase.from("restaurants").select("id").eq("slug", "wah-ji-wah").single();
  
  if (restErr || !rest) {
    console.error("WAH JI WAH restaurant not found", restErr);
    return;
  }
  ...
  await supabase.from("menu_categories").delete().eq("restaurant_id", rId);
